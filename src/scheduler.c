...
static void report_range(const SharedSearchState *shared,
                         int thread_id,
                         uint64_t start,
                         uint64_t end)
{
    pthread_mutex_lock((pthread_mutex_t *)&shared->print_mutex);
    printf("[thread %d] range=[%" PRIu64 ", %" PRIu64 ")\n",
           thread_id,
           start,
           end);
    pthread_mutex_unlock((pthread_mutex_t *)&shared->print_mutex);
}

static void report_attempt(const SharedSearchState *shared,
                           int thread_id,
